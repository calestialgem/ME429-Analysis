function v_range = VelocityRelations(vehicle)
    ...
    [a_range, ~, F_t_range, F_d_range, F_q_range, F_net_range] = vehicle.Acceleration(v_range);
    ...
    saveas(gcf, 'Acceleration vs Velocity', 'svg');
    figure();
    hold('on');
    grid('on');
    ...
    legend('F_t', 'F_d', 'F_q', 'F_{net}', 'Location', 'Best');
    saveas(gcf, 'Forces vs Velocity', 'svg');
end
